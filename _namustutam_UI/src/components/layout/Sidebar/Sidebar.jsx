...
import ContextMenu from '../../modals/common/ContextMenu/ContextMenu';
import IconPicker, { getIconByName } from '../../modals/common/IconPicker/IconPicker';
...
export default function Sidebar() {
    ...
    const fetchWorkspace = () => {
        ...
        apiClient.get(`${ENV.API_BASE_URL}/builder/data`)
            .then(res => {
    ...
    useEffect(() => {
        const url = `${ENV.API_BASE_URL}/builder/notifications/stream`;
    ...
    const handleDeleteFolder = (folder) => {
        ...
        apiClient.delete(`${ENV.API_BASE_URL}/builder/folders/${numericId}`).then(() => {
            setFolders(prev => prev.filter(f => f.id !== folder.id));
            setPages(prev => prev.filter(p => p.folderId !== folder.id));
        }).catch(_err => alert('Failed to delete folder: ' + (_err.response?.data?.error || 'Unknown error')));
    };

    const handleDeletePage = (page) => {
        if (!window.confirm(`Delete page "${page.name}"? This cannot be undone.`)) return;
        const numericId = page.id.replace('p', '');
        apiClient.delete(`${ENV.API_BASE_URL}/builder/pages/${numericId}`).then(() => {
            setPages(prev => prev.filter(p => p.id !== page.id));
        }).catch(_err => alert('Failed to delete page: ' + (_err.response?.data?.error || 'Unknown error')));
    };

    // ICON UPDATE handlers
    const handleUpdateFolderIcon = (folder, iconName) => {
        const numericId = folder.id.replace('f', '');
        apiClient.put(`${ENV.API_BASE_URL}/builder/folders/${numericId}/icon`,
            { icon: iconName }
        ).then(() => {
            setFolders(prev => prev.map(f => f.id === folder.id ? { ...f, icon: iconName } : f));
        }).catch(_err => alert('Failed to update folder icon: ' + (_err.response?.data?.error || 'Unknown error')));
    };

    const handleUpdatePageIcon = (page, iconName) => {
        const numericId = page.id.replace('p', '');
        apiClient.put(`${ENV.API_BASE_URL}/builder/pages/${numericId}/icon`,
            { icon: iconName }
        ).then(() => {
            setPages(prev => prev.map(p => p.id === page.id ? { ...p, icon: iconName } : p));
        }).catch(_err => alert('Failed to update page icon: ' + (_err.response?.data?.error || 'Unknown error')));
    };
    ...
    const handleDrop = (e, targetFolderId) => {
        ...
        apiClient.put(`${ENV.API_BASE_URL}/builder/pages/${numericPageId}/move`,
    ...
    const confirmModal = () => {
        ...
        if (modal.type === 'FOLDER') {
            apiClient.post(`${ENV.API_BASE_URL}/builder/folders`, { name: modalInput }, config)
            ...
            apiClient.post(`${ENV.API_BASE_URL}/builder/pages`, { name: modalInput, folderId: modal.targetId }, config)
                .then(res => {
            ...
            apiClient.post(`${ENV.API_BASE_URL}/builder/folders/${modal.targetId}/invite`, { email: modalInput }, config)
                .then(res => {
                    alert(`== INVITATION SECURE LINK GENERATED ==\nCopy and send this link to your teammate:\n\n${ENV.FRONTEND_URL}/invite/accept/${res.data.token}`);
                })
