                ...
                <div className="sm-modal">
                    ...
                    <div className="sm-body">
                        ...
                        <div className="sm-form-grid-4">
                            <div className="sm-form-group"><label>Order Tax ({currencySymbol})</label><input className="sm-input" type="number" min="0" step="0.01" value={form.orderTax||0} onChange={e=>setForm(f=>({...f,orderTax:e.target.value}))}/></div>
                            <div className="sm-form-group"><label>Discount ({currencySymbol})</label><input className="sm-input" type="number" min="0" step="0.01" value={form.discount||0} onChange={e=>setForm(f=>({...f,discount:e.target.value}))}/></div>
                            <div className="sm-form-group"><label>Shipping ({currencySymbol})</label><input className="sm-input" type="number" min="0" step="0.01" value={form.shipping||0} onChange={e=>setForm(f=>({...f,shipping:e.target.value}))}/></div>
                            <div className="sm-form-group"><label>Paid Amount ({currencySymbol})</label><input className="sm-input" type="number" min="0" step="0.01" value={form.paidAmount||0} onChange={e=>setForm(f=>({...f,paidAmount:e.target.value}))}/></div>
                        </div>
            ...
            {warningModalOpen && (
                <div className="sm-warning-overlay" style={{
                    ...
                    <div className="sm-warning-card" style={{
                        background: '#ffffff', borderRadius: '16px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)',
                        width: '100%', maxWidth: '500px', overflow: 'hidden', border: '1px solid #fecaca'
                    }}>
                        <div style={{
                            padding: '1.5rem 1.5rem 1rem', display: 'flex', alignItems: 'center', gap: '12px',
                            borderBottom: '1px solid #fee2e2', background: '#fef2f2'
                        }}>
                            <div style={{ backgroundColor: '#fee2e2', borderRadius: '50%', padding: '8px', display: 'flex' }}>
                                <AlertTriangle size={24} color="#dc2626" />
                            </div>
                            <div>
                                <h4 style={{ margin: 0, fontWeight: 700, color: '#991b1b', fontSize: '1.15rem' }}>Inventory Stock Warning</h4>
                                <p style={{ margin: '2px 0 0', fontSize: '0.85rem', color: '#b91c1c' }}>There are {warningProducts.length} product(s) with quantity errors</p>
                            </div>
                        </div>
                        <div style={{ padding: '1.5rem', maxHeight: '280px', overflowY: 'auto' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {warningProducts.map((p, i) => (
                                    <div key={i} style={{ background: '#fffbeb', border: '1px solid #fef3c7', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', marginTop: '6px', background: '#fee2e2', padding: '6px 10px', borderRadius: '6px', color: '#b91c1c', fontWeight: 500 }}>
                                            <span>Requested: <b>{p.quantity}</b></span>
                                            <span>Available: <b>{p.availableStock}</b></span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div style={{ padding: '1rem 1.5rem 1.5rem', display: 'flex', justifyContent: 'flex-end', background: '#f8fafc', borderTop: '1px solid #e2e8f0' }}>
                            <button onClick={() => setWarningModalOpen(false)} style={{ backgroundColor: '#dc2626', color: '#ffffff', border: 'none', borderRadius: '8px', padding: '8px 16px', fontWeight: 600, cursor: 'pointer' }}>Close & Correct</button>
                        </div>
                    </div>
                </div>
            )}
