...
const destToDest = moves.reduce((acc, m) => {
  const baseName = path.basename(m.src, path.extname(m.src)); // e.g. Login, AddCategoryModal
  acc[baseName] = m.dest; // Dest is e.g. 'pages/auth/Login.jsx'
  // also add with extension
  acc[path.basename(m.src)] = m.dest;
  return acc;
}, {});
...
function resolveImportPath(currentFile, importStr) {
  // We want to rewrite `importStr` if it points to a file that MOVED.
  // First, extract the basename of the import.
  const baseName = path.basename(importStr); // e.g. 'Login' or 'Login.css'
  
  if (destToDest[baseName]) {
    // This file was moved to `destToDest[baseName]` (which is relative to src/)
    const targetAbsPath = path.join(SRC_DIR, destToDest[baseName]);
    const currentFileDir = path.dirname(currentFile);
    
    let newRelPath = path.relative(currentFileDir, targetAbsPath);
    newRelPath = toPosixPath(newRelPath);
    if (!newRelPath.startsWith('.')) {
      newRelPath = './' + newRelPath;
    }
    
    // Check if the original import actually resolves to the moved file!
    // Since we messed up relative paths, we'll just optimistically replace based on basename match.
    // E.g., if it imports 'DeleteConfirmModal', we update the path.
    return newRelPath;
  }
  return importStr;
}
...
function fixImports(file) {
  ...
  newContent = newContent.replace(importRegex, (match, p1, p2, p3) => {
    if (p2.startsWith('.')) {
      const newPath = resolveImportPath(file, p2);
      if (newPath) {
         // Some imports don't have .jsx, we should strip .jsx if the original didn't have it.
         let finalPath = newPath;
         if (!p2.endsWith('.jsx') && finalPath.endsWith('.jsx')) {
             finalPath = finalPath.slice(0, -4);
         }
         return p1 + finalPath + p3;
      }
    }
    return match;
  });

  newContent = newContent.replace(directImportRegex, (match, p1, p2, p3) => {
    // Direct imports like `import './Login.css'`
    if (p2.startsWith('.')) {
      const newPath = resolveImportPath(file, p2);
      if (newPath) {
...
console.log('Finished deep fixing imports.');
