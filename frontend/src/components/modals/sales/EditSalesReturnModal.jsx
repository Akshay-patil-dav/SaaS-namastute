...
const EditSalesReturnModal = ({ isOpen, order, onClose, onSuccess }) => {
    ...
    const selectProduct = p => {
        ...
        if (existing) {
            setProducts(prev => prev.map(x => x.productId === p.id ? { ...x, quantity: x.quantity + 1 } : x));
        } else {
    ...
    const submit = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.error || 'Failed to update sales return.');
        ...
        <div className="srm-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
            <div className="srm-modal">
                ...
                <div className="srm-header">
                    <h4>Edit Sales Return {order?.referenceNo && <span style={{ fontSize: '13px', color: '#ff9f43', marginLeft: '8px' }}>{order.referenceNo}</span>}</h4>
                    <button className="srm-close-btn" onClick={onClose}><X size={14} /></button>
                </div>
                ...
                <div className="srm-body">
                    ...
                    <div className="srm-top-row">
                        ...
                        <div className="srm-form-group">
                            <span className="srm-label">Reference</span>
                            <input
                                className="srm-input"
                                type="text"
                            ...
                            <div className="srm-suggestions">
                                {results.length > 0 ? (
                                    <ul className="srm-suggestion-list">
                                        {results.map((r, i) => (
                                            <li key={r.id} className={`srm-suggestion-item ${i === activeIdx ? 'active' : ''}`} onClick={() => selectProduct(r)}>
                        ...
                        <table className="srm-table">
                            ...
                            <tbody>
                                {products.length > 0 ? products.map((p, idx) => (
                                    <tr key={idx}>
                                        <td>
                                            <div className="srm-table-prod-cell">
                                                {p.img ? <img src={p.img} alt="" className="srm-table-prod-img" /> : <div className="srm-table-prod-placeholder">{(p.name||'?').charAt(0)}</div>}
                                                <span className="srm-table-prod-name">{p.name}</span>
                                            </div>
                                        </td>
                                        <td><input className="srm-table-input" type="number" min="0" step="0.01" value={p.unitPrice} onChange={e => updateRow(idx, 'unitPrice', e.target.value)} /></td>
                                    ...
                                    <tr><td colSpan="8" className="srm-table-empty">No products added yet — search above to add items</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div className="srm-fields-row">
                        <div className="srm-form-group">
                            ...
                            <input className="srm-input" type="number" min="0" step="0.01" value={form.orderTax || 0} onChange={e => setForm(f => ({ ...f, orderTax: e.target.value }))} />
                        </div>
                        <div className="srm-form-group">
                            <span className="srm-label">Discount <span className="srm-required">*</span></span>
                            <input className="srm-input" type="number" min="0" step="0.01" value={form.discount || 0} onChange={e => setForm(f => ({ ...f, discount: e.target.value }))} />
                        </div>
                        <div className="srm-form-group">
                            <span className="srm-label">Shipping <span className="srm-required">*</span></span>
                            <input className="srm-input" type="number" min="0" step="0.01" value={form.shipping || 0} onChange={e => setForm(f => ({ ...f, shipping: e.target.value }))} />
                        </div>
                        <div className="srm-form-group">
                            <span className="srm-label">Status <span className="srm-required">*</span></span>
                            <select className="srm-select" value={form.status || 'Pending'} onChange={e => setForm(f => ({ ...f, status: e.target.value }))}>
